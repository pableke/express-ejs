...
function DateBox() {
	...
	this.isoEsDateTime = function(date) { return isDate(date) ? (fnEsDate(date) + " " + isoTime(date)) : null; } //dd/mm/yyyy hh:MM:ss
	...
	this.minTime = function(date) { return lpad(date.getHours()) + ":" + lpad(date.getMinutes()); } //hh:MM
	this.isoTime = function(date) { return self.minTime(date) + ":" + lpad(date.getSeconds()); } //hh:MM:ss
