...
app.use((req, res, next) => {
	...
	res.initFormError = function(form) {
		res.locals.errors = valid.initFields("/test.html").getErrors();
		return res;
	}
	res.errors = function(msg, tpl) {
...
//rutas
app.get("/", (req, res) => {
	res.initFormError(req.path).render("index", { page: "pages/index" });
});
...
app.post("/test.html", (req, res) => {
	if (!valid.validate(req.path, req.body, res.locals.i18n))
		return res.errors(res.locals.i18n.errSendMail);
	...
	mailer.send("pableke@gmail.com", req.body.asunto, tpl, res.locals)
		...
		.catch(err => res.errors(res.locals.i18n.errSendMail));
});
